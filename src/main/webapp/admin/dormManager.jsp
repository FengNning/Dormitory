...
<script type="text/javascript">
    function dormManagerDelete(dormManagerId) {
        if (confirm("您确定要删除这个宿管吗？")) {
            window.location = "dormManager?action=delete&dormManId=" + dormManagerId;
        }
    }
...
<div class="data_list">
    ...
    <form name="myForm" class="form-search" method="post" action="dormManager?action=search">
        <button class="btn btn-success" type="button" style="margin-right: 50px;" onclick="javascript:window.location='dormManager?action=preSave'">添加
        </button>
        <span class="data_search">
					<select id="searchType" name="searchType" style="width: 100px;">
                        <option value="">请选择</option>
                        <option value="managerName" ${searchType eq "managerName"?'selected':'' }>宿管姓名:</option>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-mini btn-info" type="button"
                                onclick="javascript:window.location='dormManager?action=preSave&dormManId=${dormManager.dormManId }'">
                            修改
                        </button>&nbsp;
